...
// All data from MCAP API fields — MOCK_MEMBER simulates a real API response
...
const MOCK_MEMBER = {
  ...
  // Security Property Details (Read Only)
  property: {
    unit: '',
    number: '312',
...
// ── Main component ────────────────────────────────────────────────────────────
const MembersDashboard = ({ onNavigate }) => {
  const m = MOCK_MEMBER;
  ...
  // ── Derived values ──
  const today            = new Date('2026-04-21'); // using today's date from session
    ...
    <div>
      ...
      <div style={{ background: 'var(--rh-stone-lightest)', padding: '36px 28px 56px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {/* ── Row 1: Payment breakdown + Mortgage details ── */}
          ...
          {/* ── Rate renewal alert (shown when ≤ 18 months to maturity) ── */}
          {monthsToMaturity <= 18 && (
...
Object.assign(window, { MembersDashboard });
