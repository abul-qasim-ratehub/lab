// Primitives — Button, Anchor, Card, Pill, Tag, Icon, Input, Select, CheckBullet
// Tokens come from colors_and_type.css

const Button = ({ variant = 'primary', size = 'm', children, onClick, style, ...rest }) => {
  const base = {
...
const Anchor = ({ children, icon = true, href = '#', style, ...rest }) => {
...
const Pill = ({ tone = 'lime', children, icon }) => {
  const tones = {
...
// ── Icon — loads from assets/icons/ (path relative to index.html at repo root) ──
...
const Input = ({ label, value, onChange, error, prefix, suffix, disabled, placeholder, style }) => {
...
Object.assign(window, { Button, Anchor, Pill, Icon, Card, Input, Select, CheckBullet });
