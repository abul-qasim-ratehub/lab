...
// Left: editorial headline + inline stats
// Right: floating card with icon tabs, live rate, 3-step progressive form, pulsing CTA
//
// Typography trick: "simplified." renders in italic serif for editorial contrast
// Matches the reference design pattern (NerdWallet / Bankrate style)
...
(function () {
  ...
  s.textContent = `
    ...
    .rh-hero-tab:hover { color: var(--rh-blueberry-dark) !important; }
...
// ── Exported pulse button (used across pages) ──────────────────────────────
const PulseButton = ({ children, onClick, style, size = 'l' }) => {
  const sizes = { s: { padding: '10px 20px', fontSize: 14 }, m: { padding: '13px 24px', fontSize: 15 }, l: { padding: '17px 32px', fontSize: 17 } };
    ...
    <section style={{
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 480px', gap: 64, alignItems: 'center' }}>
        ...
        <div>
          ...
          {/* Editorial headline — mixed sans bold + serif italic */}
          ...
          {/* Subheading */}
          <p style={{
            fontSize: 17, lineHeight: 1.7, color: 'var(--rh-blackberry-light)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 36, flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--rh-stone-darkest)' }}>
              <Icon name="award" size={16} color="var(--rh-yuzu-dark)" />
              MoneySense Best Platform 2024
            </div>
            <div style={{ width: 1, height: 14, background: 'var(--rh-stone)' }} />
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--rh-stone-darkest)' }}>
              <Icon name="checkmark" size={16} color="var(--rh-lime-dark)" />
              BBB A+ Accredited
            </div>
            <div style={{ width: 1, height: 14, background: 'var(--rh-stone)' }} />
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--rh-stone-darkest)' }}>
              <Icon name="maple-leaf" size={16} color="var(--rh-strawberry)" />
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    borderBottom: active ? '2px solid var(--rh-blueberry)' : '2px solid transparent',
                    marginBottom: -1,
                  }}
                >
                  <Icon name={t.icon} size={20} color={active ? 'var(--rh-blueberry-dark)' : 'var(--rh-stone-darkest)'} />
                  <span style={{ fontSize: 12, fontWeight: active ? 600 : 400, letterSpacing: '-0.01em' }}>{t.label}</span>
                </button>
              );
            })}
          </div>
